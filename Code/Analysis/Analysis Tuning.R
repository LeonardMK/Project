...
# Labeller Function
list_facet_names <- list(
  "l(X)" = TeX("$l_0(X)$"),
  "m(X)" = TeX("$m_0(X)$")
)

grid_labeller <- function(variable, value){
  list_facet_names[variable]
}
...
df_nuis_sparse_not %>% 
  ...
  stargazer(type = "latex", summary = FALSE, 
            out = "Results/Tables/Tuning/Sparse/not_eta.tex")
...
df_rate_sparse_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Sparse/not_theta.tex")

df_rate_desc_sparse_not <- list_rate_sparse_not$rate_desc

df_rate_desc_sparse_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Sparse/not_rate_theta.tex")
...
df_nuis_sparse_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE, 
            out = "Results/Tables/Tuning/Sparse/rcv_eta.tex")
...
df_rate_sparse_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Sparse/rcv_theta.tex")

df_rate_desc_sparse_rcv <- list_rate_sparse_rcv$rate_desc

df_rate_desc_sparse_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Sparse/rcv_rate_theta.tex")
...
df_nuis_sparse$Case <- factor(
  rep(vec_cases, each = 2 * int_N_unique * int_mle_unique),
  levels = vec_cases
  )

df_nuis_sparse$MLE <- str_replace(df_nuis_sparse$MLE, vec_mle_fun_unique, vec_mle)

df_nuis_sparse %>% 
  ggplot(aes(x = N, col = Case)) + 
  geom_point(aes(y = MSE), size = 1.5) + 
  geom_line(aes(y = MSE), size = 1) +
  facet_grid(Fun ~ MLE, scales = "free") + 
  labs(y = "", col = "") + 
  theme_bw()
...
df_rate_sparse <- df_rate_sparse %>% 
  mutate(algorithms = str_replace_all(algorithms, vec_mle)) %>% 
  mutate(algorithms = str_replace(algorithms, " (?=M:)", "\n"))
...
df_rate_sparse <- df_rate_sparse %>% 
  mutate(algorithms = str_replace(algorithms, "\\n", " "))
...
df_nuis_sine_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE, 
            out = "Results/Tables/Tuning/Sine/not_eta.tex")
...
df_rate_sine_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Sine/not_theta.tex")

df_rate_desc_sine_not <- list_rate_sine_not$rate_desc

df_rate_desc_sine_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Sine/not_rate_theta.tex")
...
df_nuis_sine_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE, 
            out = "Results/Tables/Tuning/Sine/rcv_eta.tex")
...
df_rate_sine_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Sine/rcv_theta.tex")

df_rate_desc_sine_rcv <- list_rate_sine_rcv$rate_desc

df_rate_desc_sine_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Sine/rcv_rate_theta.tex")
...
df_nuis_sine$Case <- factor(
  rep(vec_cases, each = 2 * int_N_unique * int_mle_unique),
  levels = vec_cases
)

df_nuis_sine$MLE <- str_replace(df_nuis_sine$MLE, vec_mle_fun_unique, vec_mle)
...
df_nuis_inter_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE, 
            out = "Results/Tables/Tuning/Inter/not_eta.tex")
...
df_rate_inter_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Inter/not_theta.tex")

df_rate_desc_inter_not <- list_rate_inter_not$rate_desc

df_rate_desc_inter_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Inter/not_rate_theta.tex")
...
df_nuis_inter_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE, 
            out = "Results/Tables/Tuning/Inter/rcv_eta.tex")
...
df_rate_inter_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Inter/rcv_theta.tex")

df_rate_desc_inter_rcv <- list_rate_inter_rcv$rate_desc

df_rate_desc_inter_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Inter/rcv_rate_theta.tex")
...
df_nuis_inter$Case <- factor(
  rep(vec_cases, each = 2 * int_N_unique * int_mle_unique),
  levels = vec_cases
)

df_nuis_inter$MLE <- str_replace(df_nuis_inter$MLE, vec_mle_fun_unique, vec_mle)
...
df_nuis_neural_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE, 
            out = "Results/Tables/Tuning/Neural/not_eta.tex")
...
df_rate_neural_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Neural/not_theta.tex")

df_rate_desc_neural_not <- list_rate_neural_not$rate_desc

df_rate_desc_neural_not %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Neural/not_rate_theta.tex")
...
df_nuis_neural_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE, 
            out = "Results/Tables/Tuning/Neural/rcv_eta.tex")
...
df_rate_neural_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Neural/rcv_theta.tex")

df_rate_desc_neural_rcv <- list_rate_neural_rcv$rate_desc

df_rate_desc_neural_rcv %>% 
  transform_scientific(int_digits) %>% 
  stargazer(type = "latex", summary = FALSE,
            out = "Results/Tables/Tuning/Neural/rcv_rate_theta.tex")
...
df_nuis_neural$Case <- factor(
  rep(vec_cases, each = 2 * int_N_unique * int_mle_unique),
  levels = vec_cases
)

df_nuis_neural$MLE <- str_replace(df_nuis_neural$MLE, vec_mle_fun_unique, vec_mle)
